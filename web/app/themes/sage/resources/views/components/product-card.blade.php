<div
x-data="productCard({ productCardData: JSON.parse( atob('{{ base64_encode(json_encode($productCardData)) }}') ) })"
class="product-card group"
>
    <div class="flex aspect-[800/1000] overflow-hidden relative bg-[#f4f2ee]">
        <a href="{{ $productCardData['permalink'] }}" title="{{ $productCardData['title'] }}" class="flex aspect-[800/1000] overflow-hidden relative bg-[#f4f2ee]">
            @if($productCardData['video'])
                <div class="bg-[#f4f2ee] flex absolute inset-0 opacity-100 group-hover:opacity-0 transition-opacity duration-500 ease-in-out">
                    <img src="{{ $productCardData['image_src']['woocommerce_single'] }}" alt="{{ $productCardData['title'] }}" class="maybeAddMixBlend object-contain w-full">
                </div>
                <video class="absolute left-0 top-0 w-full h-full object-contain opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-in-out" preload="metadata" playsinline="playsinline" loop="loop" muted="muted" autoplay>
                    <source src="{{ $productCardData['video'] }}" type="video/mp4">
                    Your browser does not support the video tag.
                </video>
            @else
                <div class="bg-[#f4f2ee] flex absolute inset-0 opacity-100 @if(!empty($productCardData['image_src_2']['woocommerce_single'])) group-hover:opacity-0 @endif transition-opacity duration-500 ease-in-out">
                    <img src="{{ $productCardData['image_src']['woocommerce_single'] }}" alt="{{ $productCardData['title'] }}" class="maybeAddMixBlend object-contain w-full">
                </div>
                @if(!empty($productCardData['image_src_2']['woocommerce_single']))
                    <div class="bg-[#f4f2ee] flex absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-in-out">
                        <img src="{{ $productCardData['image_src_2']['woocommerce_single'] }}" alt="{{ $productCardData['title'] }}" class="maybeAddMixBlend object-contain w-full">
                    </div>
                @endif
            @endif

            <div class="absolute right-1 md:right-2 top-1 md:top-2 flex gap-1 md:gap-2">
                @if($productCardData['last_units'])
                    <div class="bg-white text-[8px] md:text-[10px] leading-3 font-medium text-body p-1 md:p-2 shadow-sm">
                        {{ __('LAST UNITS','sage') }}
                    </div>
                @endif
                @if(!empty($productCardData['cross_sells']))
                    <div class="bg-white text-[8px] md:text-[10px] leading-3 font-medium text-body p-1 md:p-2 shadow-sm">
                        {{ __('PAIR OPTION AVAILABLE','sage') }}
                    </div>
                @endif
                @if(!empty($productCardData['new_in_badge']))
                    <div class="bg-white text-[8px] md:text-[10px] leading-3 font-medium text-body p-1 md:p-2 shadow-sm">
                        {{ __('NEW','sage') }}
                    </div>
                @endif
        ...
        <div class="opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-in-out flex flex-col z-10 overflow-hidden absolute left-0 bottom-0  w-full bg-white">
            ...
            <button x-on:click.prevent="addOneToCart()" type="button" name="add-to-cart" value="{{ $productCardData['product_id'] }}" 
    ...
    <div class="pt-2 md:pt-4 px-2 md:px-[18px]">
        <div class="flex justify-between gap-x-5">
            <a href="{{ $productCardData['permalink'] }}" title="{{ $productCardData['title'] }}" class="block">
        ...
        <div class="text-[10px]/3 md:text-xs/4 text-body hit-cats">
            {!! $productCardData['category_list'] !!}
        </div>
        <p class="mb-0 text-[10px]/3 md:text-xs/4 text-body">
            {!! $productCardData['price_html']  !!}
            @if($productCardData['is_on_sale']) <span class="ml-2 line-through text-red-600">{!! $productCardData['regular_price_html'] !!}</span> @endif
        </p>
    </div>
</div>
