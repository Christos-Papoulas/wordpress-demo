...
<div id="reviews" class="woocommerce-Reviews bg-[#FAFAFA] p-9">
    @if ($review_count > 0)
        <div class="text-2xl text-black">Reviews {{ $review_count }}</div>
        <div class="flex items-center">
            <div class="mr-5">
                @include('woocommerce.single-product.rating')
            </div>
            <div class="text-[44px] text-black">
                {{ $average }}
            </div>
        </div>
    @endif

    <div id="product_comments" class="">
        @foreach ($reviews as $review)
            <div class="border-b border-gray-400 py-8">
                <div class="mb-4 flex">
                    <div class="mr-3 ml-4 text-base text-black">{{ $review->comment_author }}</div>
                    <div class="text mr-3 text-black">{{ get_comment_date('', $review->comment_ID) }}</div>
                    @php
                    ...
                    @if ($rating)
                        <div class="">
                            <div class="">
                                <div class="star-rating-system" style="--rating: {{ $rating }}"></div>
                            </div>
                        </div>
                    @endif
                </div>
    ...
    <div id="review_form_wrapper" class="py-10">
        <div id="review_form">
            <?php
            ...
            $comment_form = [
                ...
                /* translators: %s is product title */
                'title_reply_to' => esc_html__('Write a Review to %s', 'woocommerce'),
                'title_reply_before' => '<span id="reply-title" class="comment-reply-title">',
                'title_reply_after' => '</span>',
                'comment_notes_after' => '',
                'label_submit' => esc_html__('Submit a Review', 'woocommerce'),
                'logged_in_as' => '',
                'comment_field' => '',
            ];
